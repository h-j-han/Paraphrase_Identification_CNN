...
function createModel(mdl, vocsize, Dsize, nout, KKw)
    ...
    if mdl == 'deepQueryRankingNgramSimilarityOnevsGroupMaxMinMeanLinearExDGpPoinPercpt' then
        ...
        --1
        incep1max:add(nn.TemporalDynamicKMaxPooling(3,0.5))
		...
		for cc = 2, ngram-1 do
            ...
            --1-
            incepMax:add(nn.TemporalDynamicKMaxPooling(kma,0.5))
		...
		--incep1min:add(nn.Reshape(NumFilter,1))		  
         incep1min:add(nn.TemporalDynamicKMinPooling(3,0.5))
		...
		for cc = 2, ngram-1 do		    
            ...
            --incepMin:add(nn.Reshape(NumFilter,1))		    		  	
             incepMin:add(nn.TemporalDynamicKMinPooling(kma,0.5))
		...
		for cc = 1, ngram-1 do
            ...
            --1-
            perConcept:add(nn.TemporalDynamicKMaxPooling(3,0.5))
		...
		for cc = 1, ngram-1 do
            ...
            perConcept:add(nn.TemporalDynamicKMinPooling(3,0.5))
			perConcept:add(nn.SpatialConvolutionMM(1,conceptFNum,1,cc)) --set
			--perConcept:add(nn.Tanh())
