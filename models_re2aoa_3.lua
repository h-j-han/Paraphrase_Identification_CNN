...
function createModel(mdl, vocsize, Dsize, nout, KKw)
        ...
        print(" small changes are made on KMaxpool on Holistic ws=1 and per-dim ws=1,2 kmax to 3")
        print(mdl)
        print(vocsize) --10000
        print(Dsize)   --300
        print(nout)    --6
    ...
    if mdl == 'deepQueryRankingNgramSimilarityOnevsGroupMaxMinMeanLinearExDGpPoinPercpt' then
        ...
        --1
        incep1max:add(nn.TemporalDynamicKMaxPooling(3,fac))
		...
		for cc = 2, ngram-1 do
            ...
            --1-
            incepMax:add(nn.TemporalDynamicKMaxPooling(kma,fac))
        ...
        incep1min:add(nn.TemporalConvolution(D,NumFilter,1,dw)) --ws = 1
		if pR == 1 then
		...
		--incep1max:add(nn.Max(1))
        --incep1max:add(nn.View())
        incep1min:add(nn.TemporalDynamicKMinPooling(3,fac))
		...
		for cc = 2, ngram-1 do
            ...
            --1-
            incepMin:add(nn.TemporalDynamicKMinPooling(kma,fac))
		...
		for cc = 1, ngram-1 do
            ...
            --1-
            perConcept:add(nn.TemporalDynamicKMaxPooling(3,fac))
		...
		for cc = 1, ngram-1 do
            ...
            perConcept:add(nn.TemporalDynamicKMinPooling(3,fac))
            perConcept:add(nn.SpatialConvolutionMM(1,conceptFNum,1,cc)) --set
			 	perConcept:add(nn.Tanh())
